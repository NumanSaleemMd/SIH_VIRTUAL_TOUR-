...
namespace IBM.Watson.Examples
{
    public class SpeechInput : MonoBehaviour
    {
        ...
        [Space(10)]
        [Tooltip("The service URL (optional). This defaults to \"https://stream-tls10.watsonplatform.net/speech-to-text/api\"" )]
        [SerializeField]
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        //Once the phrase of speech is final, use the converted text as input to the chatbot and
                        //set the processing status of the bot to "Process," and stop recording speech.
                        if (res.final)
                        {
                            Active = false;
                            bot.inputSpeech = text;
                            bot.processStatus = "Process";
                        }
                    }
